...
// Initialize arrays for storing records
$strokeRecords = [];
$bmiRecords = [];

// Set default filter values
$strokeDateFilter = $_GET['stroke_date'] ?? 'all';
$strokeRiskFilter = $_GET['stroke_risk'] ?? 'all';
$bmiDateFilter = $_GET['bmi_date'] ?? 'all';
$bmiStatusFilter = $_GET['bmi_status'] ?? 'all';
$bmiSortBy = $_GET['bmi_sort'] ?? 'newest';

// Build Stroke Records query based on filters
$strokeQuery = "SELECT RecordID, riskLevel, DetectedAt FROM health_record WHERE UserID = ?";

// Add date filter
if ($strokeDateFilter == 'week') {
    $strokeQuery .= " AND DetectedAt >= DATE_SUB(NOW(), INTERVAL 7 DAY)";
} elseif ($strokeDateFilter == 'month') {
    $strokeQuery .= " AND DetectedAt >= DATE_SUB(NOW(), INTERVAL 30 DAY)";
} elseif ($strokeDateFilter == 'quarter') {
    $strokeQuery .= " AND DetectedAt >= DATE_SUB(NOW(), INTERVAL 90 DAY)";
}

// Add risk level filter
if ($strokeRiskFilter != 'all') {
    $strokeQuery .= " AND riskLevel LIKE ?";
}

// Add sorting
$strokeQuery .= " ORDER BY DetectedAt DESC";
...
try {
    ...
    if ($strokeRiskFilter != 'all') {
        $riskParam = "%$strokeRiskFilter%";
        $strokeStmt->bind_param('ss', $userID, $riskParam);
    } else {
    ...
    while ($row = $strokeResult->fetch_assoc()) {
        $strokeRecords[] = $row;
    }
    
    $strokeStmt->close();
} catch (Exception $e) {
    error_log("Error fetching stroke records: " . $e->getMessage());
}

// Build BMI Records query based on filters
$bmiQuery = "SELECT bmiID, height, weight, bmi, detectedAt FROM bmi_record WHERE UserID = ?";

// Add date filter
if ($bmiDateFilter == 'week') {
    $bmiQuery .= " AND detectedAt >= DATE_SUB(NOW(), INTERVAL 7 DAY)";
} elseif ($bmiDateFilter == 'month') {
    $bmiQuery .= " AND detectedAt >= DATE_SUB(NOW(), INTERVAL 30 DAY)";
} elseif ($bmiDateFilter == 'quarter') {
    $bmiQuery .= " AND detectedAt >= DATE_SUB(NOW(), INTERVAL 90 DAY)";
}

// Add BMI status filter
if ($bmiStatusFilter == 'underweight') {
    $bmiQuery .= " AND bmi < 18.5";
} elseif ($bmiStatusFilter == 'normal') {
    $bmiQuery .= " AND bmi >= 18.5 AND bmi < 25";
} elseif ($bmiStatusFilter == 'overweight') {
    $bmiQuery .= " AND bmi >= 25 AND bmi < 30";
} elseif ($bmiStatusFilter == 'obese') {
    $bmiQuery .= " AND bmi >= 30";
}

// Add sorting
if ($bmiSortBy == 'oldest') {
    $bmiQuery .= " ORDER BY detectedAt ASC";
} elseif ($bmiSortBy == 'bmi_high') {
    $bmiQuery .= " ORDER BY bmi DESC";
} elseif ($bmiSortBy == 'bmi_low') {
    $bmiQuery .= " ORDER BY bmi ASC";
} else {
    $bmiQuery .= " ORDER BY detectedAt DESC";
}
...
try {
    ...
    while ($row = $bmiResult->fetch_assoc()) {
        // Add BMI status to each record
        $bmi = $row['bmi'];
        if ($bmi < 18.5) {
            $row['status'] = 'Underweight';
            $row['status_class'] = 'health-bmi-underweight';
        } elseif ($bmi < 25) {
            $row['status'] = 'Normal';
            $row['status_class'] = 'health-bmi-normal';
        } elseif ($bmi < 30) {
            $row['status'] = 'Overweight';
            $row['status_class'] = 'health-bmi-overweight';
        } else {
            $row['status'] = 'Obese';
            $row['status_class'] = 'health-bmi-obese';
        }
    ...
    error_log("Error fetching BMI records: " . $e->getMessage());
}
...
function getRiskClass($riskLevel) {
    if (stripos($riskLevel, 'low') !== false) {
        return 'health-risk-low';
    } elseif (stripos($riskLevel, 'medium') !== false || stripos($riskLevel, 'moderate') !== false) {
        return 'health-risk-medium';
    } else {
        return 'health-risk-high';
    }
}
...
function getTotalPages($items, $perPage = 10) {
    return ceil(count($items) / $perPage);
}
...
// Paginate records
$paginatedStrokeRecords = paginate($strokeRecords, $perPage, $strokePage);
$paginatedBmiRecords = paginate($bmiRecords, $perPage, $bmiPage);

// Calculate total pages
$totalStrokePages = getTotalPages($strokeRecords, $perPage);
$totalBmiPages = getTotalPages($bmiRecords, $perPage);
?>
...
<html lang="en">
  ...
  <body>
    ...
    <div class="container-fluid history-background pt-5 pb-5" data-aos="fade-in">
      <div class="health-container">
        <div class="health-header text-center mb-4 mt-3">
          <h1>Health History</h1>
          <p class="mb-0">Your health records and monitoring history</p>
        </div>

        <!-- Stroke Detection Records Section -->
        <div class="health-section mx-3" data-aos="fade-left" data-aos-delay="200">
          <div class="health-section-header d-flex justify-content-between align-items-center">
            <h2 class="mb-0">Stroke Detection Records</h2>
          </div>
          
          <div class="health-filter-container" id="strokeFilterContainer">
            <form method="get" action="" class="row g-3">
              <div class="col-md-4">
                <label class="form-label">Date Range</label>
                <select name="stroke_date" class="form-select">
                  <option value="all" <?php echo $strokeDateFilter == 'all' ? 'selected' : ''; ?>>All time</option>
                  <option value="week" <?php echo $strokeDateFilter == 'week' ? 'selected' : ''; ?>>Last 7 days</option>
                  <option value="month" <?php echo $strokeDateFilter == 'month' ? 'selected' : ''; ?>>Last 30 days</option>
                  <option value="quarter" <?php echo $strokeDateFilter == 'quarter' ? 'selected' : ''; ?>>Last 3 months</option>
                </select>
              </div>
              <div class="col-md-4">
                <label class="form-label">Risk Level</label>
                <select name="stroke_risk" class="form-select">
                  <option value="all" <?php echo $strokeRiskFilter == 'all' ? 'selected' : ''; ?>>All</option>
                  <option value="low" <?php echo $strokeRiskFilter == 'low' ? 'selected' : ''; ?>>Low Risk</option>
                  <option value="medium" <?php echo $strokeRiskFilter == 'medium' ? 'selected' : ''; ?>>Medium Risk</option>
                  <option value="high" <?php echo $strokeRiskFilter == 'high' ? 'selected' : ''; ?>>High Risk</option>
                </select>
              </div>
          ...
          <div class="health-section-body">
            <div class="table-responsive">
              <table class="table health-table">
                <thead>
                  <tr>
                    <th width="10%">#</th>
                    <th width="45%">Detected At</th>
                    <th width="45%">Risk Level</th>
                  </tr>
                </thead>
                <tbody>
                  <?php if (empty($paginatedStrokeRecords)): ?>
                    <tr>
                      <td colspan="3" class="text-center">No stroke detection records found</td>
                    </tr>
                  <?php else: ?>
                    <?php foreach ($paginatedStrokeRecords as $index => $record): ?>
                      <tr>
                        <td><?php echo ($strokePage - 1) * $perPage + $index + 1; ?></td>
                        <td><?php echo date('M j, Y h:i A', strtotime($record['DetectedAt'])); ?></td>
                        <td><span class="<?php echo getRiskClass($record['riskLevel']); ?>"><?php echo htmlspecialchars($record['riskLevel']); ?></span></td>
                      </tr>
                    <?php endforeach; ?>
                  <?php endif; ?>
                </tbody>
            ...
            <?php if ($totalStrokePages > 1): ?>
              <nav aria-label="Stroke records pagination">
                <ul class="pagination justify-content-center health-pagination">
                  <li class="page-item <?php echo $strokePage <= 1 ? 'disabled' : ''; ?>">
                    <a class="page-link" href="?stroke_page=<?php echo $strokePage - 1; ?>&stroke_date=<?php echo $strokeDateFilter; ?>&stroke_risk=<?php echo $strokeRiskFilter; ?>&bmi_page=<?php echo $bmiPage; ?>&bmi_date=<?php echo $bmiDateFilter; ?>&bmi_status=<?php echo $bmiStatusFilter; ?>&bmi_sort=<?php echo $bmiSortBy; ?>" tabindex="-1">Previous</a>
                  </li>
                  <?php for ($i = 1; $i <= $totalStrokePages; $i++): ?>
                    <li class="page-item <?php echo $i == $strokePage ? 'active' : ''; ?>">
                      <a class="page-link" href="?stroke_page=<?php echo $i; ?>&stroke_date=<?php echo $strokeDateFilter; ?>&stroke_risk=<?php echo $strokeRiskFilter; ?>&bmi_page=<?php echo $bmiPage; ?>&bmi_date=<?php echo $bmiDateFilter; ?>&bmi_status=<?php echo $bmiStatusFilter; ?>&bmi_sort=<?php echo $bmiSortBy; ?>"><?php echo $i; ?></a>
                    </li>
                  <?php endfor; ?>
                  <li class="page-item <?php echo $strokePage >= $totalStrokePages ? 'disabled' : ''; ?>">
                    <a class="page-link" href="?stroke_page=<?php echo $strokePage + 1; ?>&stroke_date=<?php echo $strokeDateFilter; ?>&stroke_risk=<?php echo $strokeRiskFilter; ?>&bmi_page=<?php echo $bmiPage; ?>&bmi_date=<?php echo $bmiDateFilter; ?>&bmi_status=<?php echo $bmiStatusFilter; ?>&bmi_sort=<?php echo $bmiSortBy; ?>">Next</a>
                  </li>
                </ul>
              </nav>
            <?php endif; ?>
        ...
        <div class="health-section mx-3" data-aos="fade-left" data-aos-delay="200">
          <div class="health-section-header d-flex justify-content-between align-items-center">
            <h2 class="mb-0">BMI Records</h2>
          </div>
          
          <div class="health-filter-container" id="bmiFilterContainer">
            <form method="get" action="" class="row g-3">
              <div class="col-md-3">
                <label class="form-label">Date Range</label>
                <select name="bmi_date" class="form-select">
                  <option value="all" <?php echo $bmiDateFilter == 'all' ? 'selected' : ''; ?>>All time</option>
                  <option value="week" <?php echo $bmiDateFilter == 'week' ? 'selected' : ''; ?>>Last 7 days</option>
                  <option value="month" <?php echo $bmiDateFilter == 'month' ? 'selected' : ''; ?>>Last 30 days</option>
                  <option value="quarter" <?php echo $bmiDateFilter == 'quarter' ? 'selected' : ''; ?>>Last 3 months</option>
                </select>
              </div>
              <div class="col-md-3">
                <label class="form-label">BMI Status</label>
                <select name="bmi_status" class="form-select">
                  <option value="all" <?php echo $bmiStatusFilter == 'all' ? 'selected' : ''; ?>>All</option>
                  <option value="underweight" <?php echo $bmiStatusFilter == 'underweight' ? 'selected' : ''; ?>>Underweight</option>
                  <option value="normal" <?php echo $bmiStatusFilter == 'normal' ? 'selected' : ''; ?>>Normal</option>
                  <option value="overweight" <?php echo $bmiStatusFilter == 'overweight' ? 'selected' : ''; ?>>Overweight</option>
                  <option value="obese" <?php echo $bmiStatusFilter == 'obese' ? 'selected' : ''; ?>>Obese</option>
                </select>
              </div>
              <div class="col-md-3">
                <label class="form-label">Sort By</label>
                <select name="bmi_sort" class="form-select">
                  <option value="newest" <?php echo $bmiSortBy == 'newest' ? 'selected' : ''; ?>>Newest first</option>
                  <option value="oldest" <?php echo $bmiSortBy == 'oldest' ? 'selected' : ''; ?>>Oldest first</option>
                  <option value="bmi_high" <?php echo $bmiSortBy == 'bmi_high' ? 'selected' : ''; ?>>BMI (high to low)</option>
                  <option value="bmi_low" <?php echo $bmiSortBy == 'bmi_low' ? 'selected' : ''; ?>>BMI (low to high)</option>
                </select>
              </div>
              <div class="col-md-3 d-flex align-items-end">
                ...
                <!-- Hidden fields to maintain stroke filters when changing BMI filters -->
                <input type="hidden" name="stroke_page" value="<?php echo $strokePage; ?>">
                <input type="hidden" name="stroke_date" value="<?php echo $strokeDateFilter; ?>">
                <input type="hidden" name="stroke_risk" value="<?php echo $strokeRiskFilter; ?>">
              </div>
            </form>
          </div>
          ...
          <div class="health-section-body">
              ...
              <table class="table health-table">
                <thead>
                  <tr>
                    ...
                    <th width="30%">Detected At</th>
                  </tr>
                </thead>
                <tbody>
                  <?php if (empty($paginatedBmiRecords)): ?>
                    <tr>
                      <td colspan="6" class="text-center">No BMI records found</td>
                    </tr>
                  <?php else: ?>
                    <?php foreach ($paginatedBmiRecords as $index => $record): ?>
                      <tr>
                        <td><?php echo ($bmiPage - 1) * $perPage + $index + 1; ?></td>
                        ...
                        <td><span class="<?php echo $record['status_class']; ?>"><?php echo $record['status']; ?></span></td>
                        <td><?php echo date('M j, Y h:i A', strtotime($record['detectedAt'])); ?></td>
                      </tr>
                    <?php endforeach; ?>
                  <?php endif; ?>
            ...
            <?php if ($totalBmiPages > 1): ?>
              <nav aria-label="BMI records pagination">
                <ul class="pagination justify-content-center health-pagination">
                  <li class="page-item <?php echo $bmiPage <= 1 ? 'disabled' : ''; ?>">
                    <a class="page-link" href="?stroke_page=<?php echo $strokePage; ?>&stroke_date=<?php echo $strokeDateFilter; ?>&stroke_risk=<?php echo $strokeRiskFilter; ?>&bmi_page=<?php echo $bmiPage - 1; ?>&bmi_date=<?php echo $bmiDateFilter; ?>&bmi_status=<?php echo $bmiStatusFilter; ?>&bmi_sort=<?php echo $bmiSortBy; ?>" tabindex="-1">Previous</a>
                  </li>
                  <?php for ($i = 1; $i <= $totalBmiPages; $i++): ?>
                    <li class="page-item <?php echo $i == $bmiPage ? 'active' : ''; ?>">
                      <a class="page-link" href="?stroke_page=<?php echo $strokePage; ?>&stroke_date=<?php echo $strokeDateFilter; ?>&stroke_risk=<?php echo $strokeRiskFilter; ?>&bmi_page=<?php echo $i; ?>&bmi_date=<?php echo $bmiDateFilter; ?>&bmi_status=<?php echo $bmiStatusFilter; ?>&bmi_sort=<?php echo $bmiSortBy; ?>"><?php echo $i; ?></a>
                    </li>
                  <?php endfor; ?>
                  <li class="page-item <?php echo $bmiPage >= $totalBmiPages ? 'disabled' : ''; ?>">
                    <a class="page-link" href="?stroke_page=<?php echo $strokePage; ?>&stroke_date=<?php echo $strokeDateFilter; ?>&stroke_risk=<?php echo $strokeRiskFilter; ?>&bmi_page=<?php echo $bmiPage + 1; ?>&bmi_date=<?php echo $bmiDateFilter; ?>&bmi_status=<?php echo $bmiStatusFilter; ?>&bmi_sort=<?php echo $bmiSortBy; ?>">Next</a>
                  </li>
                </ul>
              </nav>
            <?php endif; ?>
